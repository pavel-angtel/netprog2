...
- **Timer Configuration Register** - общие настройки таймеров;
  - Автоматический сброс счетчика аппаратного таймера Timer 0 при достижении
    порога Counter 0 (T0C0).
...
- **Interrupt Status/Clear Register** - подтверждение обработки прерывания;
  - Подтверждение обработки прерывания для Timer 0 и порога Counter 0 (T0C0).
    | Биты | Значение |          Описание          |
    |:----:|:--------:|:--------------------------:|
...
- **Interrupt Enable Register** - генерация прерываний;
  - Генерация прерывания для Timer 0 при достижении порога Counter 0 (T0C0).
    | Биты | Значение |          Описание          |
    |:----:|:--------:|:--------------------------:|
...
- **Timer 0 Compare 0 Register** - верхняя граница счетчика (порог);
  - Установка порога Counter 0 для Timer 0 (T0C0).
    | Биты |     Значение    |   Описание   |
    |:----:|:---------------:|--------------|
    | 15-0 | 0x0000 - 0xFFFF | предел счета |

- **Timer 0 Counter Register** - текущее значение счетчика аппаратного таймера;
  - Текущее значение счетчика аппаратного таймера Timer 0 (T0C0).
    | Биты |     Значение    |     Описание     |
    |:----:|:---------------:|:----------------:|
    | 15-0 | 0x0000 - 0xFFFF | текущее значение |

Смещения регистров в виртуальном адресном пространстве:
...
5. Освободить ресурсы, отменить регистрацию драйвера и деинициализировать модуль.
...
6. Ассоциирование пользовательских данных `wdt_data` с устройством `pdev`.
...
7. Получение пользовательских данных `wdt_data` из устройства `pdev`.
...
8.  Получение пользовательских данных `wdt_data` из устройства `dev`.
...
11. Изменение бита под номером `n` на значение `bit_value` в регистре.
...
12. Создание группы атрибутов файловой системы **sysfs** для устройства `pdev`.
    ...
    можно только после полной инициализации драйвера для устройства.

13. Удаление группы атрибутов файловой системы **sysfs** для устройства `pdev`.
...
14. Запись целочисленного значения в атрибут файловой системы **sysfs**.
...
15. Чтение целочисленного значения из атрибута файловой системы **sysfs**.
...
Для загрузки драйвера необходимо воспользоваться командой:

```console
$ insmod <path_to_driver>
```

Для выгрузки драйвера необходимо воспользоваться командой:
...
*/home/root*, то команды для загрузки/выгрузки драйвера будут следующими:
...
Для записи атрибута файловой системы **sysfs** разрабатываемого драйвера
необходимо воспользоваться командой:
