...
- */sys/class/net/ge1/operstate* (исключение)
...
1. Осуществить анализ аргументов командной строки с помощью [getopt(3)][1].
   ...
   - **-m** для аргумента **mtu**
   - **-l** для аргумента **link**
   - **-i** для аргумента **имя_интерфейса**

   Все аргументы являются опциональными. Аргументы **-a**, **-m** и **-l**
...
3. Изменение административного состояния сетевого интерфейса.
   - Запустите на плате программу **ping**. Команда: *ping 192.168.0.6*.
   - Измените административное состояние на **down**. Компьютер должен
     перестать получать ICMP-пакеты. Зеленый светодиод физического интерфейса
     должен погаснуть.
   - Измените административное состояние на **up**. Компьютер должен начать
