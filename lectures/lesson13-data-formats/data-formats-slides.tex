...
int main(int argc, char *argv[])
{
	...
	/* JSON_ENCODE_ANY - корневой жлемент не обязательно является объектом,
...
int main(int argc, char *argv[])
{
	...
	ellse if (json_is_array(jdata))
		/* Использование функций json_array_*. */
	ellse if (json_is_string(jdata))
		/* Использование функций json_string_*. */
	ellse if (json_is_integer(jdata))
		/* Использование функций json_integer_*. */
	ellse if (json_is_boolean(jdata))
		;
}
\stoptyping
...
-1 при добавлении двух жлементов с одинаковыми ключами.
...
перебор дочерних желементов не требуется.
\stopitemize
...
int main(int argc, char *argv[])
{
	...
	if (ret < 0) {
		/* Мы не смогли добавить jchild в объект. Надо ли его освобождать? Нет. */
		/* json_decref(jchild); */
		goto on_error;
	}
